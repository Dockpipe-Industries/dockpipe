...
set -euo pipefail
PASTE="$(dockpipe __state package-runtime --owner dorkpipe --path handoff/paste-this-prompt.txt)"
REFINED="$(dockpipe __state package-runtime --owner dorkpipe --path handoff/orchestrator-cursor-prompt.refined.md)"
if [[ ! -f "$REFINED" ]]; then
