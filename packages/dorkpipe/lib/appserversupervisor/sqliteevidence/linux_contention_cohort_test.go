...
const (
	contentionCohortOptInEnv     = "DORKPIPE_SQLITE_LINUX_CONTENTION_COHORT"
	contentionCohortChildEnv     = "DORKPIPE_SQLITE_LINUX_CONTENTION_CHILD"
	contentionCohortDatabaseEnv  = "DORKPIPE_SQLITE_LINUX_CONTENTION_DATABASE"
	contentionCohortRootEnv      = "DORKPIPE_SQLITE_LINUX_CONTENTION_ROOT"
	contentionCohortIdentityEnv  = "DORKPIPE_SQLITE_LINUX_CONTENTION_ROOT_IDENTITY"
	...
	contentionSameInitialDigest  = "bb0b0fa448e6532a65b420e128470a70fe5e32e15e94634b8c4fcf64a0b1e5ed"
	contentionSameFinalDigest    = "e024c4e5dafc3841e26abbc2df7618f2fd78fcabd3b41bd364485b2ad56ff693"
	contentionOtherInitialDigest = "8b351be57c3b6f86535ca6c2c3f6ef159175513013f7ac6608413e4e411dedfe"
	contentionOtherFinalDigest   = "5c78a969b9d47e07ef749d58c6b0fa3311512435141191d79376dd50e2f62f26"
)
...
func TestLinuxNativeSQLiteContentionCohort(t *testing.T) {
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" {
		t.Skip("Linux/amd64 native evidence only")
	}
	if !goVersionAtLeast(runtime.Version(), 1, 25) {
		t.Fatalf("contention cohort requires Go 1.25 or later; got %s", runtime.Version())
	}
	if os.Getenv(contentionCohortOptInEnv) != "1" {
		t.Skip("set DORKPIPE_SQLITE_LINUX_CONTENTION_COHORT=1 to run the 1,000-cycle contention cohort")
	}
	...
	if err != nil {
		t.Fatalf("qualify Linux contention host: %v", err)
	}
	publicationLinuxQualification = qualification
	rootIdentity, err := qualification.rootIdentity()
	if err != nil {
		t.Fatalf("capture contention fixture identity: %v", err)
	}
	...
	samePath, err := qualification.prepareEvidenceFile("main")
	if err != nil {
		t.Fatalf("prepare Linux same-session database: %v", err)
	}
	otherPath, err := qualification.prepareEvidenceFile("other")
	if err != nil {
		t.Fatalf("prepare Linux different-session database: %v", err)
	}
	...
	if got := hex.EncodeToString(initialSame.SHA256); got != contentionSameInitialDigest {
		t.Fatalf("same-session initial digest = %s, want %s", got, contentionSameInitialDigest)
	}
	if got := hex.EncodeToString(initialOther.SHA256); got != contentionOtherInitialDigest {
		t.Fatalf("different-session initial digest = %s, want %s", got, contentionOtherInitialDigest)
	}
	...
	t.Logf("sqlite version=%s source_id=%s vfs=%s uris={same=%q other=%q} pragmas={journal_mode=delete synchronous=3 fullfsync=1 temp_store=2 mmap_size=0 busy_timeout=0 foreign_keys=1 trusted_schema=0 cell_size_check=1 locking_mode=exclusive page_size=4096} schema={singleton_strict user_version=1} compile_options[%d]=%s",
		selectedSQLiteVersion, selectedSQLiteSourceID, selectedNativeVFS(), mustEvidenceURI(t, samePath), mustEvidenceURI(t, otherPath), len(sameOptions), strings.Join(sameOptions, ","))
	...
	for cycle := 1; cycle <= contentionCohortCycles; cycle++ {
		...
		owner, err := startContentionChild("owner", samePath, fixtureRoot, rootIdentity)
		if err != nil {
		...
		contenderResponse, err := runContentionOneShot("contender", samePath, fixtureRoot, rootIdentity, contentionCommand{Cycle: cycle, Operation: "expect_busy"})
		if err != nil {
		...
		busyResults++
		if contenderResponse.SQLiteCode == 5 {
			busyCode5++
		} else {
			busyCode6++
		}
		...
		otherResponse, err := runContentionOneShot("different_writer", otherPath, fixtureRoot, rootIdentity, contentionCommand{Cycle: cycle, Operation: "commit_different"})
		if err != nil {
		...
		recoveryResponse, err := runContentionOneShot("recovery", samePath, fixtureRoot, rootIdentity, contentionCommand{Cycle: cycle, Operation: "recover_old"})
		if err != nil {
		...
		writerResponse, err := runContentionOneShot("clean_writer", samePath, fixtureRoot, rootIdentity, contentionCommand{Cycle: cycle, Operation: "commit_same"})
		if err != nil {
	...
	if ownersStaged != contentionCohortCycles || protectedJournals != contentionCohortCycles || busyResults != contentionCohortCycles || busyCode5+busyCode6 != contentionCohortCycles || otherCommits != contentionCohortCycles || forcedTerminations != contentionCohortCycles || oldRecoveries != contentionCohortCycles || postRecoveryCommits != contentionCohortCycles {
		t.Fatalf("contention counters staged=%d journals=%d busy=%d code5=%d code6=%d other_commits=%d terminations=%d old_recoveries=%d same_commits=%d want=%d each",
			ownersStaged, protectedJournals, busyResults, busyCode5, busyCode6, otherCommits, forcedTerminations, oldRecoveries, postRecoveryCommits, contentionCohortCycles)
	}
	...
	if got := hex.EncodeToString(sameRow.SHA256); got != contentionSameFinalDigest {
		t.Fatalf("same-session final digest = %s, want %s", got, contentionSameFinalDigest)
	}
	if got := hex.EncodeToString(otherRow.SHA256); got != contentionOtherFinalDigest {
		t.Fatalf("different-session final digest = %s, want %s", got, contentionOtherFinalDigest)
	}
	if preTreeHash != postTreeHash {
		t.Fatalf("contention metadata tree changed: pre=%s post=%s", preTreeHash, postTreeHash)
	}
	if elapsed > 30*time.Minute {
		t.Fatalf("contention cohort elapsed %s, exceeds 30m", elapsed)
	}

	t.Logf("contention cycles=%d owner_transactions_staged=%d protected_live_journals=%d same_session_busy_or_locked=%d primary_code_5=%d primary_code_6=%d different_session_commits=%d forced_owner_terminations_before_commit_invocation=%d exact_old_row_recoveries=%d post_recovery_same_session_commits=%d ambiguous_recoveries=0 staged_row_leaks=0 revision_gaps_or_duplicates=0 digest_or_envelope_mismatches=0 unexpected_siblings_or_protection_widening=0 child_protocol_loss_duplication_substitution_or_reordering=0 retries=0 replays=0 repairs=0 fallbacks=0 inferred_acknowledgements=0",
		contentionCohortCycles, ownersStaged, protectedJournals, busyResults, busyCode5, busyCode6, otherCommits, forcedTerminations, oldRecoveries, postRecoveryCommits)
...
func TestLinuxSQLiteContentionCohortChild(t *testing.T) {
	...
	if runtime.GOOS != "linux" || runtime.GOARCH != "amd64" || evidenceCGOEnabled {
		t.Fatalf("contention child requires Linux/amd64 with CGo disabled; os=%s arch=%s cgo=%t", runtime.GOOS, runtime.GOARCH, evidenceCGOEnabled)
	}
	...
	fixtureRoot := filepath.Clean(os.Getenv(contentionCohortRootEnv))
	if fixtureRoot == "." || !filepath.IsAbs(fixtureRoot) {
		t.Fatalf("contention child fixture root must be absolute: %q", fixtureRoot)
	}
	qualification, _, err := qualifyLinuxFixtureRoot(fixtureRoot)
	if err != nil {
		t.Fatalf("qualify Linux contention child fixture: %v", err)
	}
	rootIdentity := os.Getenv(contentionCohortIdentityEnv)
	if err := qualification.validateChildPath(fixtureRoot, databasePath, rootIdentity); err != nil {
		t.Fatalf("validate Linux contention child path: %v", err)
	}
	publicationLinuxQualification = qualification
	publicationLinuxRootIdentity = rootIdentity
	t.Cleanup(func() {
	...
	case "owner":
		...
		return
	case "contender":
		response, childErr = runContentionContender(databasePath, command)
	case "different_writer":
		response, childErr = runContentionWriter(databasePath, command, contentionOtherSessionID, "commit_different")
	case "recovery":
		response, childErr = runContentionRecovery(databasePath, command)
	case "clean_writer":
		response, childErr = runContentionWriter(databasePath, command, contentionSameSessionID, "commit_same")
	default:
		t.Fatalf("unknown contention child role %q", role)
	}
	if childErr != nil {
		response = contentionResponse{Cycle: command.Cycle, Operation: command.Operation, Status: "error", Error: boundedText(childErr.Error())}
	}
...
func mustEvidenceURI(t *testing.T, path string) string {
	t.Helper()
	uri, err := evidenceFileURI(path)
	if err != nil {
		t.Fatalf("construct evidence URI for %s: %v", path, err)
	}
	return uri
}
...
func initializeContentionDatabase(t *testing.T, ctx context.Context, databasePath string, initial aggregateRow) []string {
	...
	if err := requireLinuxCompileOptions(database.compileOptions); err != nil {
		t.Fatalf("validate Linux contention compile options for %s: %v", initial.SessionID, err)
	}
	mustCreateSelectedSchema(t, ctx, database.conn)
	mustInsertAndCommitWithJournal(t, ctx, database.conn, initial, databasePath, func(t *testing.T, path string) {
		t.Helper()
		if _, err := requirePublicationJournal(path); err != nil {
			t.Fatalf("require Linux contention journal: %v", err)
		}
	})
...
func startContentionChild(role, databasePath, fixtureRoot, rootIdentity string) (*contentionChild, error) {
	command := exec.Command(os.Args[0], "-test.run=^TestLinuxSQLiteContentionCohortChild$", "-test.count=1", "-test.timeout=30m")
	command.Env = contentionChildEnvironment(role, databasePath, fixtureRoot, rootIdentity)
...
func contentionChildEnvironment(role, databasePath, fixtureRoot, rootIdentity string) []string {
	environment := make([]string, 0, len(os.Environ())+4)
	for _, item := range os.Environ() {
		if strings.HasPrefix(item, contentionCohortChildEnv+"=") || strings.HasPrefix(item, contentionCohortDatabaseEnv+"=") || strings.HasPrefix(item, contentionCohortRootEnv+"=") || strings.HasPrefix(item, contentionCohortIdentityEnv+"=") {
			continue
		}
		environment = append(environment, item)
	}
	return append(environment,
		contentionCohortChildEnv+"="+role,
		contentionCohortDatabaseEnv+"="+databasePath,
		contentionCohortRootEnv+"="+fixtureRoot,
		contentionCohortIdentityEnv+"="+rootIdentity,
	)
}

func runContentionOneShot(role, databasePath, fixtureRoot, rootIdentity string, command contentionCommand) (contentionResponse, error) {
	child, err := startContentionChild(role, databasePath, fixtureRoot, rootIdentity)
	if err != nil {
...
func requireContentionLayout(databasePath string) error {
	...
	if _, err := qualification.requirePath(filepath.Dir(databasePath), "directory", 0o700); err != nil {
		return fmt.Errorf("database directory protection: %w", err)
	}
	return qualification.requireSiblings(filepath.Dir(databasePath), false)
}
