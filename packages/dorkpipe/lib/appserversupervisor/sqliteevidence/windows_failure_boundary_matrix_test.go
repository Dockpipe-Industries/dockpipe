...
const (
	failureMatrixOptInEnv = "DORKPIPE_SQLITE_FAILURE_MATRIX"
	failureMatrixChildEnv = "DORKPIPE_SQLITE_FAILURE_MATRIX_CHILD"
	failureMatrixRows     = 22
	failureCompileHash    = "e08918d66caa484a6929317e24d92db1d6a078fc115dbf15adb10994f869babf"
)
...
var failureExpectedCompileOptions = []string{
	...
	"MAX_VDBE_OP=250000000", "MAX_WORKER_THREADS=8", "MUTEX_NOOP", "OMIT_SEH", "SOUNDEX", "SYSTEM_MALLOC",
...
func TestWindowsNativeSQLiteFailureBoundaryMatrix(t *testing.T) {
	if runtime.GOARCH != "amd64" {
		t.Skip("Windows/amd64 native evidence only")
	}
	if os.Getenv(failureMatrixOptInEnv) != "1" {
		t.Skip("set DORKPIPE_SQLITE_FAILURE_MATRIX=1 to run the deterministic failure-boundary matrix")
	}
	...
	hostFacts, err := collectAndProtectWindowsHost(matrixRoot)
	if err != nil {
		t.Fatalf("qualify failure-matrix host: %v", err)
	}
	rootPreHash, err := stablePublicationTreeHash(matrixRoot)
	if err != nil {
	...
	other := addFailureDatabase(t, ctx, contention, "different", "failure-matrix-different-session")
	...
	for attempt, fault := range []string{"schema_validation_lost", "identity_validation_lost", "digest_envelope_validation_lost", "sibling_validation_lost", "dacl_validation_lost"} {
	...
	rootPostHash, err := stablePublicationTreeHash(matrixRoot)
	if err != nil {
	...
	t.Logf("failure_matrix host={windows_build=%s arch=%s drive=%s filesystem=%s ntfs_version=%s volume=%s go=%s protection=%q} sqlite={version=%s source_id=%s vfs=%s compile_options=57 compile_sha256=%s uri=absolute_mode_rw_cache_private_txlock_exclusive_dqs_0_error_rc_1 pragmas=selected_exact schema=singleton_STRICT_user_version_1}", hostFacts.WindowsBuild, hostFacts.Architecture, hostFacts.DriveType, hostFacts.FileSystem, hostFacts.NTFSVersion, hostFacts.VolumeID, hostFacts.GoVersion, hostFacts.Protection, selectedSQLiteVersion, selectedSQLiteSourceID, selectedNativeVFS(), failureCompileHash)
...
func newFailureFixture(t *testing.T, ctx context.Context, matrixRoot, scenario string, attempt int) *failureFixture {
	...
	if err := setWindowsPrivateDirectory(root); err != nil {
		t.Fatalf("protect scenario root: %v", err)
	}
	session := fmt.Sprintf("failure-matrix-%s-%02d", scenario, attempt)
	fixture := addFailureDatabase(t, ctx, &failureFixture{root: root}, "primary", session)
	fixture.root = root
...
func addFailureDatabase(t *testing.T, ctx context.Context, parent *failureFixture, name, session string) *failureFixture {
	t.Helper()
	directory := filepath.Join(parent.root, name)
	if err := os.Mkdir(directory, 0o700); err != nil {
		t.Fatalf("create failure database directory: %v", err)
	}
	if err := setWindowsPrivateDirectory(directory); err != nil {
		t.Fatalf("protect failure database directory: %v", err)
	}
	path := filepath.Join(directory, "aggregate.sqlite")
	file, err := os.OpenFile(path, os.O_CREATE|os.O_EXCL|os.O_RDWR, 0o600)
	if err != nil {
		t.Fatalf("pre-create failure database: %v", err)
	}
	if err := file.Close(); err != nil {
		t.Fatalf("close pre-created failure database: %v", err)
	}
	if _, err := requireWindowsPrivatePath(path); err != nil {
		t.Fatalf("verify failure database protection: %v", err)
	}
	...
	return &failureFixture{root: parent.root, database: path, session: session, oldRow: oldRow, newRow: newRow, compileSet: append([]string(nil), database.compileOptions...)}
}
...
func (fixture *failureFixture) command(cycle, attempt int, operation, checkpoint string) failureCommand {
	return failureCommand{Scenario: filepath.Base(fixture.root), Cycle: cycle, Attempt: attempt, Operation: operation, Checkpoint: checkpoint, Database: fixture.database, Session: fixture.session}
}
...
func (fixture *failureFixture) result(t *testing.T, scenario string, attempt int, boundary, evidence, classification string, final aggregateRow) failureMatrixResult {
	...
	return failureMatrixResult{Scenario: scenario, Attempt: attempt, Boundary: boundary, Evidence: evidence, Classification: classification, Initial: fixture.oldRow, Final: final, PreTree: fixture.preTree, PostTree: post}
}

func executeFailureCommand(t *testing.T, command failureCommand) failureResponse {
	t.Helper()
	base := failureResponse{Scenario: command.Scenario, Cycle: command.Cycle, Attempt: command.Attempt, Operation: command.Operation, Checkpoint: command.Checkpoint, Database: command.Database, Session: command.Session, Status: "ok", NativeKind: "none", NativeDetail: "no SQLite result observed", Classification: "rejected"}
...
func requireFailureCompileOptions(t *testing.T, got []string) {
	t.Helper()
	if len(got) != 57 || len(failureExpectedCompileOptions) != 57 || strings.Join(got, "\n") != strings.Join(failureExpectedCompileOptions, "\n") {
...
func failureValidationFault(attempt int) string {
	faults := []string{"schema_validation_lost", "identity_validation_lost", "digest_envelope_validation_lost", "sibling_validation_lost", "dacl_validation_lost"}
...
func startFailureChild(t *testing.T) *failureChild {
	t.Helper()
	command := exec.Command(os.Args[0], "-test.run=^TestWindowsSQLiteFailureMatrixChild$", "-test.count=1", "-test.timeout=30m")
	command.Env = failureChildEnvironment()
...
func failureChildEnvironment() []string {
	environment := make([]string, 0, len(os.Environ())+1)
	for _, item := range os.Environ() {
		if strings.HasPrefix(item, failureMatrixChildEnv+"=") {
			continue
		}
		environment = append(environment, item)
	}
	return append(environment, failureMatrixChildEnv+"=1")
}
...
func requireFailureResponse(t *testing.T, command failureCommand, response failureResponse) {
	t.Helper()
	if response.Scenario != command.Scenario || response.Cycle != command.Cycle || response.Attempt != command.Attempt || response.Operation != command.Operation || response.Checkpoint != command.Checkpoint || response.Database != command.Database || response.Session != command.Session {
...
func mustStableFailureTree(t *testing.T, root string) string {
	t.Helper()
	hash, err := stablePublicationTreeHash(root)
	if err != nil {
