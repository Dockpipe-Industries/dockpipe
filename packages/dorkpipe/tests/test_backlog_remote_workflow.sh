...
test "$validation_input_file_count" -eq 182
...
remote_diff_fingerprint="$(sed -n 's/.*"remote_diff_fingerprint": "\([^"]*\)".*/\1/p' "$fixture_root/validation-receipt.json")"
remote_result_fingerprint="$(sed -n 's/.*"remote_result_fingerprint": "\([^"]*\)".*/\1/p' "$fixture_root/validation-receipt.json")"
compatibility_fingerprint="$(sed -n 's/.*"compatibility_fingerprint": "\([^"]*\)".*/\1/p' "$fixture_root/validation-receipt.json")"
test -n "$remote_diff_fingerprint"
test -n "$remote_result_fingerprint"
test -n "$compatibility_fingerprint"
grep -Fq "\"fingerprint\": \"$remote_diff_fingerprint\"" "$artifact_root/validation-receipt.json"
grep -Fq "\"fingerprint\": \"$remote_result_fingerprint\"" "$artifact_root/validation-receipt.json"
...
grep -Fq "\"compatibility_fingerprint\": \"$compatibility_fingerprint\"" "$artifact_root/validation-receipt.json"
