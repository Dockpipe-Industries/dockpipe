...
require_text "$EXECUTION" 'Required one-shot readiness proof is missing, partial, or stale' "missing one-shot readiness proof"
require_text "$EXECUTION" 'Exact non-consuming preflight fails before invocation' "failed one-shot preflight"
...
require_text "$HANDOFF" 'status: verified_current | unverified | not_applicable' "one-shot readiness vocabulary"
require_text "$EXECUTION" 'status: verified_current | unverified' "execution admits one-shot readiness record"
require_text "$HANDOFF" 'Never turn `unverified` into `verified_current` through wording or compression.' "readiness cannot be invented during handoff"
require_text "$HANDOFF" 'Name checkpoint roles separately' "distinct checkout and promotion checkpoints"
require_text "$EXECUTION" 'never assume those commits must be equal' "distinct checkpoint validation"
require_text "$EXECUTION" 'validate the remaining pre-mutation predicates' "fail-fast repair covers downstream checks"
