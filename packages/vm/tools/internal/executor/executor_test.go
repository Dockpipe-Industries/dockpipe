...
func executorFixture(t *testing.T) Contract {
	...
	runtime := filepath.Join(root, "runtime", "run-001", "cohort-001")
	...
	qemuSystem := provisioning.PinnedCommand{ToolID: provisioning.ToolQEMUSystem, Binary: filepath.Join(root, "toolchain", "bin", provisioning.ToolQEMUSystem), BinarySHA256: strings.Repeat("2", 64), Version: "QEMU emulator version 11.0.3", Args: []string{"-nodefaults", "-nic", "none"}, TimeoutSeconds: 120}
	c := Contract{
...
func TestBuildDerivesClosedExecutorContractFromAuthorizedPlan(t *testing.T) {
	...
	c := provisioning.Contract{
		...
		Roots:       provisioning.Roots{Instances: filepath.Join(root, "instances"), Evidence: filepath.Join(root, "evidence"), Config: filepath.Join(root, "config"), Runtime: filepath.Join(root, "runtime")},
	...
	p := provisioning.Plan{Schema: provisioning.PlanSchema, ContractSHA256: contractSHA, ToolchainSHA256: c.Toolchain.ManifestSHA256, RunID: c.RunID, CohortID: c.CohortID, LiveAuthorized: true, AuthorizationRequired: true, Operations: operations}
	...
	qemuPlan, err := manifest.PlanProvisioningQEMU(m, filepath.Join(c.Roots.Runtime, c.RunID, c.CohortID), osDisk, filepath.Join(instance, "qualification.raw"), filepath.Join(instance, "nocloud-seed.iso"))
	if err != nil {
		t.Fatal(err)
	}
