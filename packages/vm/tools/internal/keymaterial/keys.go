...
func Generate(root, name string) (Pair, error) {
	...
	publicFile, err := os.OpenFile(pair.PublicPath, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0o600)
	if err != nil {
		return Pair{}, fmt.Errorf("create public key: %w", err)
	}
	if _, err := publicFile.Write(pub); err != nil {
		publicFile.Close()
		return Pair{}, err
	}
