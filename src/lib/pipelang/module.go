...
func hasArithmeticResultSourceContract(contract LanguageContract) bool {
	if contract == PipeLangLanguageContractV230 || contract == PipeLangLanguageContractV220 || contract == PipeLangLanguageContractV210 {
		return true
	}
...
func hasPrimitiveRecordListAtSourceContract(contract LanguageContract) bool {
	return contract == PipeLangLanguageContractV200 || contract == PipeLangLanguageContractV210 || contract == PipeLangLanguageContractV220 || contract == PipeLangLanguageContractV230
}

func hasPrimitiveRecordListFindByTextSourceContract(contract LanguageContract) bool {
	return contract == PipeLangLanguageContractV210 || contract == PipeLangLanguageContractV220 || contract == PipeLangLanguageContractV230
}

func hasPrimitiveRecordListFilterByTextSourceContract(contract LanguageContract) bool {
	return contract == PipeLangLanguageContractV220 || contract == PipeLangLanguageContractV230
}

func hasCaseFoldedTextContainmentSourceContract(contract LanguageContract) bool {
	return contract == PipeLangLanguageContractV230
}
...
func prepareModuleGraph(input ModuleSetInput, sources *SourceSet, requireSemanticIDs bool) (*ModuleGraph, Diagnostics) {
	...
	if strings.TrimSpace(string(input.LanguageContract)) == "" || strings.TrimSpace(string(input.LanguageContract)) != string(input.LanguageContract) {
		...
		diagnostics = append(diagnostics, moduleDiagnostic(CodeInvalidModule, Span{}, fmt.Sprintf("semantic analysis requires a supported post-legacy language contract through %q", PipeLangLanguageContractV230)))
	}
