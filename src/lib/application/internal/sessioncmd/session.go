// Package sessioncmd owns the dockpipe session command surface.
package sessioncmd
...
// Run dispatches the runtime-owned Git session command.
...
func cmdSessionList(args []string) error {
	...
	for _, session := range sessions {
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\n",
			session.SessionID,
			textvalue.FirstNonBlank(session.Status, "-"),
			textvalue.FirstNonBlank(session.Repo.SessionRef, "-"),
			textvalue.FirstNonBlank(session.Storage.Workspace, "-"),
			textvalue.FirstNonBlank(session.UpdatedAt, session.CreatedAt, "-"),
		)
	}
	return tw.Flush()
}
...
func cmdSessionWorkerAcquire(args []string) error {
	...
	fmt.Fprintf(os.Stdout, "Mode:      %s\n", textvalue.FirstNonBlank(lease.Mode, "-"))
	fmt.Fprintf(os.Stdout, "Volume:    %s\n", textvalue.FirstNonBlank(lease.Volume, "-"))
	return nil
}
...
func shellChangeDirectoryCommand(path string) string {
	...
	return "cd " + shellquote.POSIX(path)
}
