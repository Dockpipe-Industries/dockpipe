// Package pipelangmaterialize compiles PipeLang authoring files into staged workflow artifacts.
...
// ExistingRoots returns unique absolute existing directories in input order.
...
// MaterializeRoots compiles every PipeLang entry found under the existing roots.
func MaterializeRoots(roots []string, force bool, outBase string) (int, error) {
	total := 0
	for _, root := range ExistingRoots(roots) {
		root := root
...
func materializePipeLangFile(path, root string, force bool, outBase string) (bool, error) {
	...
	moduleRoot := DetectModuleRoot(entryDir)
	files, latestMod, err := ReadFilesUnder(moduleRoot)
	if err != nil {
		return false, err
	}
...
// InferEntryClassFromTypeRef resolves a concrete class from a class or interface type reference.
...
// DetectModuleRoot finds the nearest PipeLang module config above startDir.
func DetectModuleRoot(startDir string) string {
	dir := startDir
	for {
...
// ReadFilesUnder returns all PipeLang source files and their latest modification time.
func ReadFilesUnder(root string) (map[string][]byte, time.Time, error) {
	out := map[string][]byte{}
