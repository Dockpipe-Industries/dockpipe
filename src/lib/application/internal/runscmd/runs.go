// Package runscmd owns the dockpipe runs inspection command.
package runscmd
...
// Run inspects host-run, policy, and operation-event records.
...
func cmdRunsEvents(eventLog, eventIndex string, jsonOut bool) error {
	...
	for _, event := range events {
		fmt.Fprintf(os.Stdout, "%s  %-8s  %s", textvalue.FirstNonBlank(event.Timestamp, "-"), textvalue.FirstNonBlank(event.Status, "-"), textvalue.FirstNonBlank(event.Unit, "-"))
