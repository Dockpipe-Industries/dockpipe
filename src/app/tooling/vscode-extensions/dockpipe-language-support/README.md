...
- PipeLang editor support understands interface/object field types and generic list shapes such as `List<string>` and `List<IImageResource>`. It also highlights and completes the explicitly versioned arithmetic Result spellings `Result<int, ArithmeticError>` and `Result<float, ArithmeticError>`; `v0.2.0` admits direct checked addition, `v0.3.0` additionally admits direct checked subtraction, `v0.4.0` additionally admits direct checked multiplication, `v0.5.0` additionally admits direct checked integer negation, `v0.6.0` additionally admits direct checked binary64 division, and `v0.7.0` admits an identical Result parameter/return with that parameter as the complete method body. `v0.8.0` preserves those contracts and admits `<`, `<=`, `>`, and `>=` only for the exact two-parameter ordinal `string` ordering method shape; the existing grammar already highlights those operators. Model-field traversal remains limited to the frozen data-model surface; Result construction, extraction, propagation, matching, hashing, normalization, and broader text APIs are not suggested.
