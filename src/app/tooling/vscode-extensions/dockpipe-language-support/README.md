...
- PipeLang editor support understands interface/object field types and generic list shapes such as `List<string>` and `List<IImageResource>`. It also highlights and completes the explicitly versioned arithmetic return spellings `Result<int, ArithmeticError>` and `Result<float, ArithmeticError>`; `v0.2.0` admits direct checked addition, `v0.3.0` additionally admits direct checked subtraction, `v0.4.0` additionally admits direct checked multiplication, `v0.5.0` additionally admits direct checked integer negation, and `v0.6.0` additionally admits direct checked binary64 division. Model-field traversal remains limited to the frozen data-model surface.
