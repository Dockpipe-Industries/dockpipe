...
- PipeLang editor support understands interface/object field types and generic list shapes such as `List<string>` and `List<IImageResource>`. It also highlights and completes the explicitly versioned arithmetic return spelling `Result<int, ArithmeticError>`; `v0.2.0` admits direct checked addition, `v0.3.0` additionally admits direct checked subtraction, `v0.4.0` additionally admits direct checked multiplication, and `v0.5.0` additionally admits direct checked integer negation. Model-field traversal remains limited to the frozen data-model surface.
